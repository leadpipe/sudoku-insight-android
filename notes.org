...
** TODO Use a date as the seed for generating puzzles
   - Hash the following things together for basic seed:
     - Generation stream number
     - Today's date, or one day after the previous date used, no more than 30 days ago
     - A counter for the date
     - The generator (symmetry preserving or not)
     - "Properness" flag (allows >1 solution or not)
       - improper puzzles need a bunch of other support
   - Use this to choose a symmetry
