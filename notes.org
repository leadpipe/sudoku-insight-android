* Insights
** Evaluator
   - straight shot: analyze, apply best move, track time
     - results: elapsed time, grid-marks, status (complete, error, inconclusive), interruption
   - top-level evaluation:
     - do straight shot
     - if not complete, do N disproof runs and average their times
   - disproof run:
     - choose impossible assignment, do error-search run, eliminate the assignment, do straight shot
     - if not complete, loop
   - error-search run:
     - do straight shot
     - if ends in error, done
     - choose set of assignments
       - for each assignment, apply it and do error-search run
