...
- [ ] have InsightMeasurer emit info about errors: need to compute likelihood of seeing errors
  - every move made while an error is present counts as not having seen
  - every trail abandoned with an error counts as having seen it
  - every undo from an erroneous state to a good state counts as having seen it
