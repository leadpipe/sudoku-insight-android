...
* App
** TODO Bug: Don't use onPause/onResume to pause and resume the game.  See javadoc for onResume.
...
** TODO Handle the importing of collections of puzzles
   ...
   - Still want to generate new puzzles: add a pref for how often to do this, as
     ...
     occasionally, etc)
** DONE Add a replay timestamp; display it on the info page
** DONE Add "Skipped" as a game state, and "Skip" as a menu option (in place of Give Up) before the first move
** DONE Add voting on (solved) puzzles: +1 or -1 (or 0).  Allow update from info page.
   - Add sort by fun, show +1 votes first, oldest
   - Reload list on resume; for tablet, reload list on vote
   - If reloading the list is too awful we can get more sophisticated later
