...
*** TODO New evaluator algorithm [2/6]
- [X] Rework InsightMeasurer to track buckets of insights
  - bucket axes: easy/hard, and direct/simply-implied/complexly-implied
  - [X] divide by realm -- let's look at block/line/location as independent bits
  - [X] list each bucket containing a move corresponding to assignment made
    - each bucket has realm vector and scan-target count vector (indexed by realm)
- [X] Rework ScanPoints to keep pace, and measure params
  - Unfortunately, this demonstrates that these buckets do not form anything
    like a Poisson process.
  - On the plus side, we see that minOpen (low-water-mark for unset locations,
    across all trails) has a nice correlation with the mean of the very Poisson
    time-waiting-before-starting-new-trail.
  - Also noteworthy: the fact that the IMPLIED bucket has _zero_ instances where
    it was the only bucket describing the move made.  This means that the only
    cases where that actually happened, I started a trail.
- [ ] Another thing to try: trailhead time per minOpen
- So try a different approach:
- [ ] Try replicating the old way, to some extent: find everything and include
  the total target count.  However, count by min kind of the move made, not max
  of mins of all moves.
- [ ] Or, try just easy-direct and simply-easy (with expanded def of easy).
- [ ] (This is now called into question.  The premise of this approach appears to be false.)
...
*** DONE Fix up some things before running next experiments for evaluator improvements [3/3]
- [X] Look for all hidden sets before any naked ones
- [X] Fix the "easy" test
  - [X] need to track which units are involved in eliminations, to tell which units are required
  - [X] s/b any unit with 6 assignments, not just the block
  - [X] try out rule change: >=6 assignments in one unit, OR 2 units sufficient to cover all possibilities
- [X] Fix the "simple" test: a hidden set can only be in the outermost implication
