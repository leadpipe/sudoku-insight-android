* Insights
** Evaluator
*** DONE Try removing most obvious correlations from InsightMeasurer
...
*** TODO Evaluate puzzles while (or before) adding them
    - make a dedicated background thread that generates and evaluates puzzles well in advance
    - have the info and list pages interact with this thread similarly to the network thread
*** TODO Add a dialog that shows the evaluation (in some form) and lets you skip the puzzle
    - use an overlay instead of a dialog, so menu works
    - make it slightly harder to skip?
...
*** TODO Show stars in title
*** TODO Use stars instead of number in list
*** TODO Update help
