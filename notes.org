...
*** TODO I think the one-move-at-a-time approach is unworkable
- look into using multiple moves found by a single scan
*** TODO Look into separating kinds of scan points
- blocks, blocks+lines, blocks+lines+locations
- doesn't appear to be worth it?
- let's investigate this some more, especially in light of having to minimize everything
*** TODO Minimize everything in the evaluator
- need this given only counting scan targets in assignments
*** TODO Look for all hidden sets before any naked ones
*** TODO Performance improvements [0/2]
- [ ] Run profiler after minimization change
