...
*** TODO Fix up some things before running next experiments for evaluator improvements [0/3]
- [ ] Look for all hidden sets before any naked ones
- [ ] Fix the "easy" test
  - [ ] need to track which units are involved in eliminations, to tell which units are required
  - [ ] s/b any unit with 6 assignments, not just the block
  - [ ] try out rule change: >=6 assignments in one unit, OR 2 units sufficient to cover all possibilities
- [ ] Fix the "simple" test: a hidden set can only be in the outermost implication
