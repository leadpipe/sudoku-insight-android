...
/**
 * Analyzes the output from {@link DetailGrabber} to figure out the relative
 * ease of different insights.
 */
public class DetailSusser {
  public static void main(String[] args) throws IOException {
    BufferedReader in = new BufferedReader(new FileReader("details.txt"));
    ...
    for (String line; (line = in.readLine()) != null; ) {
      ...
      Pattern pattern = Pattern.fromString(iter.next());
      Universe universe = new Universe(iter);

      Reporter reporter = getReporter(minOpen, numTrails);
      double seconds = ms / 1000.0;
      reporter.take(isTrailhead, seconds, openCount, pattern, universe);
      if (iter.hasNext()) {
        in.close();
  ...
  static class Reporter {
    ...
    public void take(boolean isTrailhead, double seconds, int openCount, Pattern pattern, Universe universe) {
      BatchProcess process = processes.getUnchecked(Sp.fromPattern(pattern, openCount));
      if (isTrailhead)
        process.takeTrailhead(seconds, openCount, universe);
      else
        process.take(seconds, openCount, universe);
    }
  }
  ...
  static class BatchProcess {
    ...
    public void report(PrintStream out) {
      out.printf("%s: %,d moves, %,d trailheads (%.2f%% seen)\n",
          sp, movesIncluded, trailheadsIncluded,
          100.0 * movesIncluded / (movesIncluded + trailheadsIncluded));
